        ...
        for s_index in range(self.a_hat.shape[0]):
            ...
            for f_index in range(self.a_hat.shape[1]):
                if self.a_hat[s_index, f_index] != 0:
                    tmp_t_range = []
                    for t_index in np.arange(0, t_max, 0.1):
...
class CustomSolver:
    ...
    def count_alpha(self) -> np.ndarray:
        alpha: np.ndarray = np.zeros(self.a_hat.shape)
        alpha[self.mask] = 1 + np.log(
            1 + self.a_hat[self.mask] * self.i_p_hat[self.mask] ** 2
        )
        return alpha
    ...
    def count_beta(self) -> np.ndarray:
        ...
        beta[self.mask] = (
            np.exp(
                -2
                * (
                    0.5 * (self.i_p_hat[self.mask] + self.i_d_hat[self.mask])
                    - self.i_t_hat[self.mask]
                )
            )
            * self.a_hat[self.mask]
        )
        return beta
    ...
    def count_gamma(self) -> np.ndarray:
        ...
        gamma[self.mask] = (
            0.5
            * (1 + self.alpha[self.mask])
            * (1 + self.beta[self.mask]) ** 2
            * (
                1
                + (1 + self.a_hat[self.mask] ** 2)
                / (1 + self.i_d_hat[self.mask] * self.i_p_hat[self.mask])
            )
        )
        return gamma
    ...
    def i_d(self, t: float) -> np.ndarray:
        ...
        output[self.mask] = (
            self.i_d_hat[self.mask]
            * (1 + self.alpha[self.mask] + self.gamma[self.mask] + self.beta[self.mask])
            ** 2
            * (1 + self.alpha[self.mask] ** 2 * t) ** 0.5
        ) / 64
        output[output > 1] = 1
        return output
    ...
    def i_t(self, t: float) -> np.ndarray:
        output: np.ndarray = np.zeros(self.a_hat.shape)
        output[self.mask] = self.i_t_hat[self.mask] * np.exp(
            -0.5
            * t
            * (self.alpha[self.mask] + self.beta[self.mask] + self.gamma[self.mask])
        )
    ...
    def eta(self, t: float) -> np.ndarray:
        output: np.ndarray = np.zeros(self.a_hat.shape)
        output[self.mask] = 1 - np.log(
            1 + (self.alpha[self.mask] + 260) * self.i(t)[self.mask]
        )
        return output
